...
my_cshp <- function (date = NA)
{
    ...
    cshp.part <- cshp.full[startdate <= date & enddate >=
                           date, ]
    ## slightly thin (simplify) the polygons for faster plotting
...
for (year in start:end){
    ...
    breaks <- seq(-maximum, maximum,2); labels <- breaks
    ...
    map <- ggplot(map.df) + aes(long, lat, group=group, fill=GDPgrowth) +
        ...
        scale_fill_gradient2(mid = 'yellow', low='red4',
                             high = 'green4',
                             breaks= breaks, limits=c(-maximum, maximum),
                             labels= labels) +
    ...
    timeline <- ggplot(wb.world[1:match(year, wb.world$date), ],
        ...
        annotate('rect', ymin = seq(-ncolors - increment/2, ncolors - increment/2, increment),
                 ymax = seq(-ncolors + increment/2, ncolors + increment/2, increment),
                 xmin = -Inf, xmax = Inf, fill=colors) +
        annotate('rect', ymin = ncolors, ymax = Inf,
                 xmin = -Inf, xmax = Inf, fill=colors[length(colors)]) +
        geom_line() + geom_point() +
        scale_x_continuous(breaks = seq(1960, 2016, 5)) +
        scale_y_continuous(limits=c(-3, 8), breaks=seq(-2, maximum, 2),
                           expand=c(0,0)) +
